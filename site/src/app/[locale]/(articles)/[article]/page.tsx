...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    getArticles().map((a) => ({ locale, article: a.slug })),
  );
}
      ...
      <header className="relative flex min-h-[440px] items-end overflow-hidden sm:min-h-[560px] lg:min-h-[640px]">
        ...
        <Container width="full" className="relative pb-14 pt-40 sm:pb-20">
          <div className="max-w-4xl">
            <Link
              href={cheminLocalise(routes.newsCategory, locale)}
              className="rise text-[12px] font-semibold uppercase tracking-[0.2em] text-gold"
            >
              {d.actualites.article.categorie}
            </Link>
            <h1
      ...
      {related.length > 0 && (
        <Section tone="surface">
          <Container width="full">
            <Eyebrow>{d.actualites.article.aLireAussi}</Eyebrow>
            ...
            <div className="mt-10">
              <ArticlesCarousel
                ...
                locale={locale}
                libelles={{
                  lireArticle: d.commun.lireArticle,
